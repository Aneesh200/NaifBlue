...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get the project reference from the URL
    const projectRef = supabaseUrl.match(/(?:db|supabase)\.([a-z0-9-]+)/i)?.[1] || '';
    console.log('Project ref (POST):', projectRef);
    ...
    console.log('Available cookies (POST):', cookieNames);
    ...
    for (const cookieName of possibleCookieNames) {
      const cookie = request.cookies.get(cookieName);
      if (cookie) {
        console.log('Found auth cookie (POST):', cookieName);
        accessToken = cookie.value;
        break;
      }
    }
    ...
    if (!accessToken) {
      for (const cookie of request.cookies.getAll()) {
        if (
          ...
          console.log('Found potential auth cookie (POST):', cookie.name);
          accessToken = cookie.value;
          break;
        }
      }
    ...
    // Special handling for code-verifier cookies
    if (!accessToken || accessToken.includes('verifier')) {
      console.log('No valid token found, checking authorization header... (POST)');
      
      // Try to get from Authorization header
      const authHeader = request.headers.get('authorization');
      if (authHeader && authHeader.startsWith('Bearer ')) {
        accessToken = authHeader.substring(7);
        console.log('Found token in Authorization header (POST)');
      }
    }
    
    if (!accessToken || accessToken.includes('verifier')) {
      console.log('No Supabase auth token found in cookies (POST)');
      return NextResponse.json(
        { error: 'Unauthorized' },
        { status: 401 }
      );
    ...
    try {
      ...
      if (tokenValue.startsWith('{') && tokenValue.endsWith('}')) {
        ...
        if (tokenData.access_token) {
          tokenValue = tokenData.access_token;
          console.log('Extracted access_token from JSON cookie (POST)');
        }
      }
    } catch (e) {
      console.error('Error parsing token JSON (POST):', e);
      // Not JSON, continue with the token as is
    }
      ...
      if (tokenValue.includes('.')) {
        ...
        if (tokenParts.length === 3) {
          try {
            ...
            userId = payload.sub;
            console.log('Extracted user ID from token (POST):', userId);
          } catch (error) {
            console.error('Error decoding JWT (POST):', error);
          }
        }
      }
    } catch (error) {
      console.error('Error parsing token (POST):', error);
    }
